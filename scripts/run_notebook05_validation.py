...
This script preserves the notebook logic but makes it easier to run on a remote
machine via terminal / tmux / Docker.
"""
...
# Local helper from your project
import cuzr_setup_multi as cz
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        choices=["dev", "prod"],
        default="dev",
        help="dev ~= laptop/smoke-scale settings; prod ~= remote/A100 settings",
    )
    parser.add_argument(
        "--project-path",
        default="../cu_zr_mlip_project",
        help="pyiron project path",
    )
    parser.add_argument(
        "--results-dir",
        default="outputs/notebook05_validation",
        help="Directory for CSVs, plots, and exported artifacts",
    )
    parser.add_argument(
        "--pots",
        default="all",
        help=(
            "Comma-separated potential ids to run, or 'all'. Example: "
            "MACE_A,MACE_B,EAM_Mendelev_2019_CuZr"
        ),
    )
    parser.add_argument(
        "--include-ace",
        action="store_true",
        help="Register commented ACE placeholders too",
    )
    parser.add_argument(
        "--cores",
        type=int,
        default=None,
        help="Override number of cores passed to pyiron/LAMMPS jobs",
    )
    parser.add_argument(
        "--project-tag",
        default=None,
        help="Optional tag to append to exported files",
    )
...
def register_potentials(include_ace: bool) -> List[Dict[str, Any]]:
    potential_specs = [
...
def ncl_validation_pipeline(
    ...
    j0 = cz.run_static(
        pr,
        jname("static_glass_seed", pot_spec, mode_dev),
        structure0,
        pot_spec,
        cores=cores,
        delete_existing=False,
    ...
    jm = cz.run_md(
        pr=pr,
        job_name=jname("melt", pot_spec, mode_dev),
    ...
    for T in quench_ts:
        jq = md_from_last(
            pr=pr,
            job_name=jname(f"quench_{T}K", pot_spec, mode_dev),
            prev_job=prev,
            pot_spec=pot_spec,
            T=T,
            steps=steps_each,
    ...
    jmin = run_minimize(
        pr,
        jname("glass_min", pot_spec, mode_dev),
    ...
    jnve = run_nve(
        pr,
        jname("glass_nve", pot_spec, mode_dev),
    ...
    jcomp = cz.run_static(
        pr=pr,
        job_name=jname("glass_compressed", pot_spec, mode_dev),
...
def vacancy_formation_energy(
    ...
    jperf = run_minimize(
        pr,
        jname(f"vac_perfect_{structure_name}", pot_spec, mode_dev),
        perfect,
        pot_spec,
        cores=cores,
    )
    ...
    jdef = run_minimize(
        pr,
        jname(f"vac_defect_{structure_name}", pot_spec, mode_dev),
        defect,
        pot_spec,
        cores=cores,
    )
...
def main() -> int:
    ...
    all_pots = register_potentials(include_ace=args.include_ace)
    ...
    # 3. NCL-oriented validation pipeline
    ncl_rows = []
    for p in pots_to_run:
        print("RUNNING NCL PIPELINE:", p["id"])
        ncl_rows.append(
            ncl_validation_pipeline(
                pr=pr,
                structure0=glass_seed,
                pot_spec=p,
                mode_dev=mode_dev,
                cores=cores,
                T_melt=T_melt,
    ...
    # 4. Bulk modulus estimates from EOS
    structure_map = {
        "FCC_Cu": fcc_cu,
        "HCP_Zr": hcp_zr,
        "B2_CuZr": b2_cuzr,
    ...
    for p in pots_to_run:
        ...
        for sname, structure, rep in vacuum_targets:
            try:
                vac_rows.append(
                    vacancy_formation_energy(pr, structure, sname, p, mode_dev=mode_dev, cores=cores, repeat=rep)
                )
    ...
    for p in pots_to_run:
        ...
        try:
            ...
            print(f"RDF / S(q) skipped for {pid}: {exc}")
        ...
        summary_rows.append(
            {
                ...
                "glass_pipeline_done": bool(pid in set(ncl_df["pot_id"])),
                "rdf_available": pid in rdf_store,
                "nve_energy_shift_per_atom_eV": get_nve_drift_for_potential(pid, ncl_df),
            }
        )
    ...
    print("\nDone.")
    print(f"pyiron project: {pr.path}")
    print(f"results dir:    {results_dir.resolve()}")
    print("\nSelected potentials:")
    print(", ".join([p["id"] for p in pots_to_run]))
    return 0
