...
from src.data import (  # noqa: E402
    normalize_config_dataset_paths,
    resolve_dataset_paths,
    sanity_check_paths,
)
from src.path_utils import resolve_path  # noqa: E402
from src.ace_runner import run_ace_training  # noqa: E402
...
def main() -> int:
    ...
    if args.output_root:
        ...
        cfg["run"]["output_root"] = str(
            resolve_path(cfg["run"]["output_root"], base_dir=config_dir)
        )
    ...
    meta = {
        ...
        "resolved_datasets": {
            k: str(v) if v is not None else None for k, v in dataset_paths.items()
        },
    ...
    if return_code != 0:
        print(f"ACE training failed with exit code {return_code}", file=sys.stderr, flush=True)
        return return_code
    return 0
