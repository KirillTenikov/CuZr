\
    ...
    def parse_args() -> argparse.Namespace:
        ...
        p.add_argument(
            "--forces-key",
            default="auto",
            help="Force key in Atoms.arrays. Use 'auto' to try common names.",
        )
        p.add_argument(
            "--e0-json",
            default=None,
    ...
    def resolve_energy_key(atoms: Atoms, requested: str) -> str:
        if requested != "auto":
            if requested not in atoms.info:
                raise KeyError(f"Requested energy key '{requested}' not found. Available info keys: {list(atoms.info.keys())}")
            return requested
        for key in AUTO_ENERGY_KEYS:
            if key in atoms.info:
                return key
        raise KeyError(f"Could not auto-detect energy key. Available info keys: {list(atoms.info.keys())}")


    def resolve_forces_key(atoms: Atoms, requested: str) -> str:
        if requested != "auto":
            if requested not in atoms.arrays:
                raise KeyError(f"Requested forces key '{requested}' not found. Available array keys: {list(atoms.arrays.keys())}")
            return requested
        for key in AUTO_FORCE_KEYS:
            if key in atoms.arrays:
                return key
        raise KeyError(f"Could not auto-detect forces key. Available array keys: {list(atoms.arrays.keys())}")
    ...
    def convert_split(
        ...
        for at in frames:
            ...
            if forces.shape != (len(at), 3):
                raise ValueError(f"Forces for one structure have shape {forces.shape}, expected ({len(at)}, 3).")
