\
    ...
    BASIS_PROFILES: Dict[str, Dict[str, Dict[str, List[int]]]] = {
        ...
        "medium": {
            "UNARY": {"nradmax_by_orders": [16, 7, 5, 3, 2, 2], "lmax_by_orders": [0, 3, 3, 2, 2, 1]},
            "BINARY": {"nradmax_by_orders": [16, 7, 4, 3, 2, 1], "lmax_by_orders": [0, 3, 2, 2, 1, 0]},
            "TERNARY": {"nradmax_by_orders": [16, 4, 3, 2, 1], "lmax_by_orders": [0, 2, 2, 1, 1]},
            "ALL": {"nradmax_by_orders": [16, 4, 3, 2, 1], "lmax_by_orders": [0, 2, 2, 1, 1]},
        },
        "large": {
            "UNARY": {"nradmax_by_orders": [18, 8, 5, 4, 3, 2], "lmax_by_orders": [0, 4, 3, 2, 2, 1]},
            "BINARY": {"nradmax_by_orders": [18, 8, 5, 3, 2, 2], "lmax_by_orders": [0, 3, 3, 2, 1, 1]},
            "TERNARY": {"nradmax_by_orders": [18, 5, 4, 2, 2], "lmax_by_orders": [0, 3, 2, 1, 1]},
            "ALL": {"nradmax_by_orders": [18, 5, 4, 2, 2], "lmax_by_orders": [0, 3, 2, 1, 1]},
        },
        "xlarge": {
            "UNARY": {"nradmax_by_orders": [20, 10, 6, 4, 3, 2], "lmax_by_orders": [0, 4, 4, 3, 2, 1]},
            "BINARY": {"nradmax_by_orders": [20, 10, 6, 4, 3, 2], "lmax_by_orders": [0, 4, 3, 2, 2, 1]},
            "TERNARY": {"nradmax_by_orders": [20, 6, 5, 3, 2], "lmax_by_orders": [0, 3, 3, 2, 1]},
            "ALL": {"nradmax_by_orders": [20, 6, 5, 3, 2], "lmax_by_orders": [0, 3, 3, 2, 1]},
        },
    }
    ...
    def build_pacemaker_input(cfg: Dict[str, Any], run_dir: Path) -> Dict[str, Any]:
        ...
        potential = {
            ...
            "embeddings": {
                "ALL": {
                    "npot": model_cfg.get("embedding", {}).get("npot", "FinnisSinclairShiftedScaled"),
                    "fs_parameters": model_cfg.get("embedding", {}).get("fs_parameters", [1, 1, 1, 0.5]),
        ...
        input_cfg: Dict[str, Any] = {
            ...
            "backend": {
                ...
                "batch_size": int(backend_cfg.get("batch_size", train_cfg.get("batch_size", 100))),
